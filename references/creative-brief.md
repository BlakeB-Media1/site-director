...
1. **Harvest before asking.** Pull answers from: the invocation message,
   ...
   the project, your client-context folder (offer docs, runbooks, personas)
   when the client is an agency client, and the existing
...
- Local-service preset: on | off                # local-service overrides
...
- Pages: <route — purpose — conversion goal>   # or "tier: starter|pro|premium" w/ local-service preset
...
| Field | Default | Why |
|---|---|---|
| Stack | Recipe A; Recipe B if local-service preset on | registry-native vs agency template |
