...
Run `node <SKILL_DIR>/scripts/gate-check.mjs <PROJECT_DIR>` +
impeccable's slop detector (`node <impeccable-skill-dir>/scripts/detect.mjs`).
...
| Gate | Rule | Source |
|---|---|---|
| Animation properties | no animation/transition on width/height/top/left/margin/padding/border/font-size — transform/opacity/clip-path/filter only; `prefers-reduced-motion` fallback present | rules/web/coding-style |
| Semantic HTML | div count ≤ 5× semantic elements (header/nav/main/section/article/aside/footer) | stitch gate 8b |
| Anti-template | page shows ≥4 of the 10 design qualities (below); centered-hero + gradient blob + uniform cards + gray-on-white = instant fail | rules/web/design-quality |
...
| Accessibility | axe: zero serious/critical; visible focus; contrast AA (4.5:1 body, 3:1 large); labels on all inputs; alt on all imgs | rules/web/testing |
| Build | recipe build exits 0 | — |
| Forms | primary conversion path works end-to-end (submit → success state) | CRO floor |
| Bundle | landing ≤150kb JS gz (Next) / microsite ≤80kb (Astro agency); CSS ≤30kb | rules/web/performance |
| CWV | LCP <2.5s, INP <200ms, CLS <0.1 (Lighthouse, local) | rules/web/performance |
...
```
for N in 1..3:
  run gate-check.mjs + detect.mjs → .site-director/gate-results-N.json
